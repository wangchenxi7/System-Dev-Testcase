 ...
 *    Need to launch a daemon thread to pool the fd events.
 *    When a page fault is triggerred, kernel will inform the daemon thread via the fd event.
 * 
 * 		[?] Will the kernel inform the user space which page triggered the page fault ?
 * 
 * 
 */
...
int main(int argc, char* argv[]){
  ...
  if (ioctl(fd, UFFDIO_REGISTER,  &reg)) {
	...
	ptr = user_buf;
	for(i=64; i<user_buffer_size; i+=(4096 +64) ){
		fprintf(stderr, "Trigger fault page thread: Page[%d] (addr 0x%llx) : %s \n", i/4096, (uint64_t)(ptr+i), ptr );
	}
