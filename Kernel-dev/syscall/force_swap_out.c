...
int main(){
	...
	uint64_t request_addr 	= 0x400100000000; // start of RDMA data space
	...
	uint64_t size  					=	0x200000; // 2MB, a single PMD entry
	char* user_buff;
	uint64_t i;
	...
	//
	type = 0x4;
	printf("Force swap out [0x%llx, 0x%llx]\n", request_addr, request_addr + size );
  ...
  printf("System call id SYS_do_semeru_rdma_ops, type 0x%x, memory server %d, returned %d \n", type, target_server, syscall_ret);
	...
	printf("After force swap-out, read the first uint64_t of the user_buffer: 0x%llx \n",*(uint64_t*)user_buff);
