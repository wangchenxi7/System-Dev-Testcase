...
#include <sys/mman.h>  
#include <sys/errno.h>
//#include </usr/include/asm-generic/mman-common.h> // duplicated with <sys/mman.h> 
...
int main(){
	...
	unsigned long request_addr		= 0x400100000000UL; // Dara regions, Java heap of Semeru
	//unsigned long size  					=	0x100000;		// 1MB, for unsigned long, length is 0x20,000
	unsigned long size  					=	0x20000000;		// 512MB, for unsigned long, length is 0x200,000
	char* user_buff;
	unsigned long i;
	unsigned long sum = 0;
	unsigned long swapped_out_pages = 0;
