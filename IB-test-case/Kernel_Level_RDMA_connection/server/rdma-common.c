...
/**
 * [?] Free memory and evict memory to disk ?
 * 
 */
  ...
  while (running) {// server is working
    ...
    if (filtered_free_mem_g < FREE_MEM_EVICT_THRESHOLD){   
      ...
      if (evict_hit_count >= MEM_EVICT_HIT_THRESHOLD){
        ...
        stop_size_g = FREE_MEM_EVICT_THRESHOLD - last_free_mem_g;  // The memory size to be evicted to disk ?
...
void post_receives(struct connection *conn)
{
  ...
  sge.addr    = (uintptr_t)conn->recv_msg;   // [?] The value of this mesage ??
...
/**
 * Send a registered memory chunk to CPU server/client.
