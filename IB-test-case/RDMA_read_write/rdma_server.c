...
struct app_context{
	struct ibv_context 		*context;   // points to the opened InfiniBand device
	struct ibv_pd      		*pd;
	struct ibv_mr      		*mr;    		// Registered memory region, used for rdma_read/write. Only the Reciever needs to register this and then send to Sender ?
	struct ibv_cq      		*rcq;       // reciever completion queue
	struct ibv_cq      		*scq;				// sender completion queue
	struct ibv_qp      		*qp;        // queue pair 
	struct ibv_comp_channel *ch;
	void               		*buf;				// Got a buffer(memory region) from OS and register it to device ??
	unsigned            	size;       // size of buf
	int                 	tx_depth;
	struct ibv_sge      	sge_list;	  // [?]
	struct ibv_send_wr  	wr;					// [?] work request, store flags ?? 
};


// Works like ip, all the IB connection information
struct ib_connection {
    int             	lid;
    int            	 	qpn;
    int             	psn;
	unsigned 						rkey;
	unsigned long long 	vaddr;       // The registered virtual address, read/write data from here?
};
...
struct app_data {
	int							port;                					// IP port
	int							ib_port;                      // active infiniband port
	unsigned        size;                         //[?] message size ??  
	int             tx_depth;                     //[?] ?
	int 		    		sockfd;
	char						*servername;
	struct ib_connection		local_connection;     // [?] Why does it need to record local_connection information ?
	struct ib_connection 		*remote_connection;
	struct ibv_device			*ib_dev;                // [?] Record device information ?
...
int main(int argc, char *argv[])
{
  ...
  // Works like package header ?
	// Why not initialize  app_data->the local_connection ?
	struct app_data	 	 data = {
		.port	    		  		= 19765,  		// tcp/ip port.  We can use the port of qperf
		.ib_port    				= 1,					// hardware port of IB, 2 ports for zion's ConnectX-3
		.size       				= 65536,			// Local/Remote buffer size ?
		.tx_depth   				= 100,				//  ?
		.servername 				= NULL,
	...
	/*
	* What's the fisrt parameter ?
	* arg 0 : rdma  // Just a keyword ? 
	* arg 1 : 
	*/


	if(argc == 2){
		data.servername = argv[1];  // local hostname or remote hostname ?
	}
	if(argc > 2){
	...
	if(data.servername){   // client path  : Build a TCP connect with the server
		data.sockfd = tcp_client_connect(&data);   	
	}else{								// Server path  : listen on port : data->port
	...
	if(data.servername){
		qp_change_state_rtr(ctx->qp, &data);   // client 
	}else{
		qp_change_state_rts(ctx->qp, &data);   // server
	}	

	if(!data.servername){     //server path
		/* Server - RDMA WRITE */
		...
		char *chPtr = ctx->buf;   // write the data to be sent in context->buf
...
static struct app_context *init_ctx(struct app_data *data)
{
  ...
  TEST_Z(data->ib_dev = dev_list[0],
	...
	TEST_Z(ctx->context = ibv_open_device(data->ib_dev),
			"Could not create context, ibv_open_device");               // ibv_context is something got by openning IB device ?
	
	TEST_Z(ctx->pd = ibv_alloc_pd(ctx->context),
			"Could not allocate protection domain, ibv_alloc_pd");      // allocate a Protect Domain for a ibv_context. 
	...
	// Create the IB communication information
	// [?] Why does here need 2 (send/recieve) completion queue ? 
	// [?] What's the use of qp (queue pair) ?
...
static void rdma_write(struct app_context *ctx, struct app_data *data){
	...
	ctx->sge_list.addr      = (uintptr_t)ctx->buf;
  	...
  	ctx->wr.wr.rdma.remote_addr = data->remote_connection->vaddr;   // write server_ctx->buf into ctx->remote_server->ctx->mr ??
