...
struct app_context{
	...
	struct ibv_pd      		*pd;
	struct ibv_mr      		*mr;    		// memory region to be read/wrritten by IB ? 
	struct ibv_cq      		*rcq;
	struct ibv_cq      		*scq;
	struct ibv_qp      		*qp;
	struct ibv_comp_channel *ch;
	void               		*buf;				// Got a buffer from OS and register it to ibv_mr.
	unsigned            	size;
	int                 	tx_depth;
...
struct ib_connection {
    ...
    int             	psn;
	unsigned 						rkey;
	unsigned long long 	vaddr;     //[?] What's the connection between app_data->ib_connection->vaddr and app_context->buf ??
};
...
static struct app_context *init_ctx_with_buffer(struct app_data *data, void* buffer);
...
int main(int argc, char *argv[])
{
	...
	struct app_data	 	 data = {
		...
		.size       				= BUFFER_SIZE,		// Local/Remote buffer size ?
		.tx_depth   				= 100,						//  ?
		.servername 				= NULL,
		.remote_connection 	= NULL,						// Store the IB, buffer etc. information of remote servers. 
		.ib_dev     				= NULL
	...
	//debug
	// Build a big buffer and register it as ibv_mr
	void* client_buf = (void*)malloc(BUFFER_SIZE * 2);   
	memset(client_buf, 0, BUFFER_SIZE * 2);

	TEST_Z(ctx = init_ctx_with_buffer(&data, client_buf),
...
static struct app_context *init_ctx_with_buffer(struct app_data *data, void* buffer)
{
	...
	ctx->buf = buffer;										// asign it into the context
	ctx->size = BUFFER_SIZE;
	TEST_NZ(posix_memalign(buffer, page_size, BUFFER_SIZE * 2),  // why double the size ?
				"could not allocate working buffer ctx->buf");
	memset(buffer, 0, BUFFER_SIZE * 2);  	// reset the buffer 
  ...
  TEST_Z(data->ib_dev = dev_list[0],
	...
	TEST_Z(ctx->mr = ibv_reg_mr(ctx->pd, buffer, ctx->size * 2, 
				IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE),
			"Could not allocate mr, ibv_reg_mr. Do you have root access?");
...
static void rdma_write(struct app_context *ctx, struct app_data *data){
  	...
  	ctx->sge_list.length    = ctx->size;
   	...
   	// The work request link list which should be put into the send queue.
